<?php

// READ INPUT

$myfile = fopen("day06_input.txt", "r") or die("Unable to open file!");
$banks = trim(fgets($myfile));
fclose($myfile);

// END READ INPUT

// Start timing
$start_time = microtime(true);

// Create array from string and make integers of all elements
$clean_banks = explode("\t", $banks);
$clean_banks = array_map("intval", $clean_banks);

// Initiate variables
$prev_states = array();
$banks_count = count($clean_banks);
$last_state = json_encode($clean_banks);
$i = 0;

// Loop until last state is found amongst all previous states
while (!in_array($last_state, $prev_states)) {
    // Add last state to previous states
    $prev_states[] = $last_state;

    // Find the bank with largest amount of blocks, find index of that bank and
    // set that bank to 0
    $max_blocks = max($clean_banks);
    $max_pos = array_search($max_blocks, $clean_banks);
    $clean_banks[$max_pos] = 0;

    // Find how many new blocks all banks will get and blocks that will only be
    // given to banks "after" the current
    $add_all = floor($max_blocks / $banks_count);
    $add_rest = $max_blocks % $banks_count;

    // Add amount to all banks if we should
    if ($add_all > 0) {
        foreach ($clean_banks as $key => $value) {
            $clean_banks[$key] = $value + $add_all;
        }
    }

    // Add 1 to banks after current, add_rest shows how many banks will get
    // more blocks
    $k = $max_pos + 1;
    for ($j = 0; $j < $add_rest; $j++) { 
        // If we reach the end, go to the beginning
        $k = ($k > $banks_count-1) ? 0 : $k;
        $clean_banks[$k]++;
        $k++;
    }

    // Encode the current state of all banks, since we will compare it with
    // previous states and it's easier to have a string to compare with
    $last_state = json_encode($clean_banks);

    // Increase our counter
    $i++;
}

// Find the index first occurance of the last state (the one that ended the
// loop), the counter i minus that index is our cycle amount
$first_repeat = array_search($last_state, $prev_states);
$cycle_length = $i - $first_repeat;

// Stop timing
$end_time = microtime(true);

//var_dump($prev_states);

// Print results
echo("Result: " . $cycle_length . "\n");
echo("Execution time: " . ($end_time - $start_time) . " seconds");

?>
