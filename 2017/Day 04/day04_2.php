 ...
 */
function isAnagram(string $str_one, string $str_two) {
 ...
 */
function checkForAnagrams(array $pass_split) {
    ...
    for ($i = 0; $i < $pass_length_short; $i++) {
        for ($j = $i + 1; $j < $pass_length; $j++) {
            ...
            if (isAnagram($pass_split[$i], $pass_split[$j])) {
                return true;
            }
        }
    }
...
foreach ($passwords_arr as $pass) {
    ...
    if (!checkForAnagrams($pass_split)) {
        $correct++;
    }
}
